/*---------------------- 삼면보드  ------------------------- */
function vannerImg() {
  window.open("banner_img", "display_nf", "width=1000,height=800");
}
function vanner_boardValue() {
  ...
  let urlWithParams = "banner?" + urlParams.toString();

  window.open(urlWithParams, "display_nf", "width=1000,height=800");
}
...
function horizontal_boardValue(button) {
  ...
  let bib = parentParentElement.children[2].children[0].value;
  let rank = parentParentElement.querySelector("#rank").value;
  let highest = parentParentElement.querySelector("#result").value;
  let round = document.querySelector("#round").value;
  let gender = document.querySelector("#gender").value;
  let trial = document.querySelector("#period").value;
  ...
  if (periodInput) {
    ...
    for (let i = 0; i < participants_information.length; i++) {
      ...
      if (participant["record"].length > periodValue) {
        record = participant["record"][periodValue];
      }
    }
  }
  let urlParams = new URLSearchParams({
  ...
  let urlWithParams = "board?" + urlParams.toString();
...
function Vertical_boardValue(button) {
  ...
  let Bib = parentTrElement.children[2].children[0].value;
  let highest = parentTrElement.querySelector("#result").value;
  let rank = parentTrElement.querySelector("#rank").value;
  let sportsName = document.querySelector("#sport").value;
  let round = document.querySelector("#round").value;
  let gender = document.querySelector("#gender").value;
  let trial = document.querySelector("#period").value;
  let pic = "";
  ...
  let vertical_height = heightValue;
...
function boardList() {
  ...
  let urlWithParams = "board_list?" + urlParams.toString();
...
// function AjaxData(jsonData, url, id, view) {
//   $.ajax({
//     type: "post",
//     url: url,
...
//   });
// }
function InputValueCheckColor(input) {
...
function TrackJson(participants) {
  ...
  participants.forEach((participant) => {
    const bibJSON = {
      ...
      wind: participant.Wind,
      reactiontime: participant.Reaction,
      highest: participant.highest,
      medal: participant.Medal,
      bigo: participant.Bigo,
      newrecord: participant.Newrecord,
...
function get_participants_information() {
  ...
  const Player_Order = document.querySelectorAll("#order");
  ...
  highJumpData(Player_Order, order);
  ...
  for (let i = 0; i < participant_BIB.length; i++) {
    ...
    participant_dictionary["order"] = order[i];
...
function SendVerticalData(url, id, view) {
  const jsonData = convertParticipantsToJSON(rankcal2());
  AjaxData(jsonData, url, id, view);
}
/*---------------------- 기록 관련 순위 매기는 로직  ------------------------- */
...
function get_horizontal_field_exclude_wind_game_information() {
  ...
  for (let i = 0; i < participants_data.length; i++) {
    ...
    participant_information["order"] =
      participant.querySelector("#order").value;
...
function Send_Throw_Data(url, id, view) {
  const jsonData = convertParticipantsToJSON(rankcal());
  AjaxData(jsonData, url, id, view);
}

function Send_horizontal_Data(url, id, view) {
...
function updateBestScoreAndWind(UserInput, top, wind) {
  ...
  let endIndex = UserInput.children.length - 4;
  ...
  for (let i = 5; i < endIndex; i++) {
    ...
    if (parsedTop < parsedChildValue) {
      ...
      if (TableLength) {
        ...
        console.log(UserInput.nextElementSibling.children[i - 5]);
        wind =
          UserInput.nextElementSibling.children[i - 5].firstElementChild.value;
      }
    }
    ...
    if (TableLength) {
      ...
      if (document.querySelector("#Qq") !== null) {
        ...
        UserInput.children[
          UserInput.children.length - 4
        ].firstElementChild.value = top;
        UserInput.nextElementSibling.children[
          UserInput.children.length - 9
        ].firstElementChild.value = wind;
        if (isNMScore) {
          UserInput.children[
            UserInput.children.length - 4
  ...
  for (let i = 5; i < endIndex; i++) {
    ...
    if (TableLength) {
      ...
      if (document.querySelector("#Qq") !== null) {
        ...
        console.log(UserInput.children[UserInput.children.length - 4]);
        UserInput.children[
          UserInput.children.length - 4
        ].firstElementChild.value = top;
...
function SendTrackData(url, id, view) {
  const participants_data = document.getElementsByTagName("tbody")[1].children;
  const participants_information = [];
  for (let j = 0; j < participants_data.length; j++) {
    ...
    for (let j = 5; j < endIndex; j++) {
      console.log(participant.children[j]);
...
function SendRelayData(url, id, view) {
  const participants_data = document.getElementsByTagName("tbody")[1].children;
  const participants_information = [];
